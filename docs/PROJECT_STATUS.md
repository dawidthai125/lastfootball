...
**PRODUCTION VERIFIED · GREEN** — platforma + First Match + Hub SEASON + liga + finanse + **Players Thin Slice**.  
Baseline: **`0b960b5`** (LFE-PLAYERS-01) · https://lastfootball.vercel.app
...
| Platform       | Landing · Auth · Club Wizard · First Match · Hub · liga · finanse · kadra  |
| Product design | GDD-01…15 CLOSED                                                           |
| Infra          | Vercel + Supabase `anoeimngwptucjdugjme` + fixtures + cash + **players**   |
...
- **LFE-PLAYERS-01** — **CLOSED** · tabela `players` · `resolveClubSquad(rows)` · prod `0b960b5`
...
1. GDD-16+ (Owner wybiera rozdział) **lub** Transfers (GDD §12) na bazie `players`.
2. GDD §26 (liczby ekonomiczne — zastąpią `ECONOMY_THIN`).
3. Training (§8) — po odblokowaniu treningu / statusach kadry.
